...
import { hierarchicalChunker, Chunk } from '../../rag/chunking.js';
...
// --- Test Configuration ---
const TEST_CHROMA_URL = process.env.TEST_CHROMA_URL || 'http://localhost:8000';
...
const testIndexerRef = chromaIndexerRef({ collectionName: TEST_COLLECTION_NAME });

// Helper function to setup test directory and files
async function setupTestFiles() {
  await fs.mkdir(TEST_PROJECT_DIR, { recursive: true });
  // .gitignore
  await fs.writeFile(path.join(TEST_PROJECT_DIR, '.gitignore'), 'ignored.txt\nnode_modules/\n');
  // Text file
  await fs.writeFile(path.join(TEST_PROJECT_DIR, 'doc1.txt'), 'This is the first test document.');
  // Markdown file with code block
  await fs.writeFile(path.join(TEST_PROJECT_DIR, 'doc2.md'), '# Markdown Doc\n\nSome text here.\n\n```javascript\nconst greeting = "hello world";\nconsole.log(greeting);\n```\n\nMore text.');
  // Code file
  await fs.writeFile(path.join(TEST_PROJECT_DIR, 'code1.ts'), 'export function add(a: number, b: number): number {\n  return a + b;\n}');
  // Ignored file
  await fs.writeFile(path.join(TEST_PROJECT_DIR, 'ignored.txt'), 'This file should be ignored.');
  // File in ignored directory
  await fs.mkdir(path.join(TEST_PROJECT_DIR, 'node_modules'));
  await fs.writeFile(path.join(TEST_PROJECT_DIR, 'node_modules', 'lib.js'), 'console.log("ignored lib");');
}
...
async function indexTestFileContent(filePath: string, sourceIdentifier: string): Promise<void> {
    const content = await fs.readFile(filePath, 'utf-8');
    const extension = path.extname(filePath);
    const chunks = hierarchicalChunker(content, extension);
    chunks.forEach(chunk => {
        if (!chunk.metadata) chunk.metadata = {};
        chunk.metadata.sourcePath = sourceIdentifier; // Use sourcePath
    });
    // We need the initialized 'ai' instance here for ai.embed
    // Let's ensure genkit is initialized before this helper is called
    // Get the initialized instance by calling genkit with the imported config
    const ai = genkit(config);
    if (chunks.length > 0) {
        await ai.index({ indexer: testIndexerRef, documents: chunks });
    }
}
...
describe('RAG Flows - E2E Tests', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      testCollection = await chromaClient.getOrCreateCollection({ name: TEST_COLLECTION_NAME });
      console.log(`[E2E Test] Ensured test collection exists: ${TEST_COLLECTION_NAME}`);
      ...
      console.error("[E2E Test Setup Error] Failed to connect to ChromaDB or setup test environment.", error);
      console.error(`Ensure ChromaDB is running at ${TEST_CHROMA_URL} and accessible.`);
  ...
  afterAll(async () => {
    ...
    if (chromaClient && TEST_COLLECTION_NAME) {
      try {
        ...
        if (collections.includes(TEST_COLLECTION_NAME)) {
            ...
            console.log(`[E2E Test] Deleted test collection: ${TEST_COLLECTION_NAME}`);
        } else {
            console.log(`[E2E Test] Test collection ${TEST_COLLECTION_NAME} not found for deletion (might have been cleared).`);
        }
      } catch (error) {
        console.error(`[E2E Test Cleanup Error] Failed to delete or check collection ${TEST_COLLECTION_NAME}:`, error);
      }
    }
    // Cleanup test files
  ...
  beforeEach(async () => {
    ...
    try {
        ...
        console.error(`[E2E Test beforeEach Error] Failed to clear collection ${TEST_COLLECTION_NAME}:`, error);
    }
  });
  ...
  it('should correctly index different file types with appropriate metadata', async () => {
    ...
    await indexTestFileContent(path.join(TEST_PROJECT_DIR, 'doc1.txt'), sourceTxt);
    await indexTestFileContent(path.join(TEST_PROJECT_DIR, 'doc2.md'), sourceMd);
    await indexTestFileContent(path.join(TEST_PROJECT_DIR, 'code1.ts'), sourceTs);
    ...
    // Need to import IncludeEnum from chromadb
    const results = await testCollection.get({ include: [IncludeEnum.Metadatas, IncludeEnum.Documents] });
    ...
    console.log(`[E2E Test] Found ${txtChunks.length} chunks for ${sourceTxt}`);


    // 2. Check .md file chunks (should have text and code)
    const mdChunksMeta = metadatas.filter(m => m?.sourcePath === sourceMd);
    const mdChunksDocs = documents.filter((doc, idx) => metadatas[idx]?.sourcePath === sourceMd);
    ...
    console.log(`[E2E Test] Found ${mdTextChunks.length} text chunks and ${mdCodeChunks.length} code chunks for ${sourceMd}`);
    ...
    console.log(`[E2E Test] Found ${tsChunks.length} chunks for ${sourceTs}`);
    ...
    const ignoredChunks = metadatas.filter(m => ignoredSources.includes(m?.sourcePath || ''));
  ...
  it('should retrieve only text chunks when filtering by contentType: text', async () => {
    ...
    console.log(`[E2E Test] Query with filter ${JSON.stringify(filter)} returned (excerpt): ${result.substring(0, 100)}...`);
  }, 10000);
  ...
  it('should list the source paths of all indexed documents', async () => {
    ...
    // Act
    const paths = await listDocumentsFlow(); // No projectId needed
  ...
  it('should remove all chunks associated with a specific document source path', async () => {
    ...
    await indexTestFileContent(path.join(TEST_PROJECT_DIR, 'doc2.md'), pathToRemove);
    await indexTestFileContent(path.join(TEST_PROJECT_DIR, 'doc1.txt'), remainingPath1);
    ...
    const results = await testCollection.get({ where: { sourcePath: pathToRemove } });
  ...
  it('should remove all documents from the collection', async () => {
    ...
    await indexTestFileContent(path.join(TEST_PROJECT_DIR, 'doc1.txt'), path1);
    await indexTestFileContent(path.join(TEST_PROJECT_DIR, 'code1.ts'), path2);
    await new Promise(resolve => setTimeout(resolve, 200)); // Delay
  ...
  it('should specifically retrieve content from a markdown code block', async () => {
    ...
    await indexTestFileContent(path.join(TEST_PROJECT_DIR, 'doc2.md'), 'test-doc2.md-reindexed');
    ...
    console.log(`[E2E Test] Query for markdown code block returned (excerpt): ${result.substring(0, 100)}...`);
  }, 10000);
  ...
  it('should retrieve both text and code chunks when no filter is applied', async () => {
     ...
     await indexTestFileContent(path.join(TEST_PROJECT_DIR, 'doc1.txt'), 'test-doc1.txt-reindexed');
     await indexTestFileContent(path.join(TEST_PROJECT_DIR, 'doc2.md'), 'test-doc2.md-reindexed');
     await indexTestFileContent(path.join(TEST_PROJECT_DIR, 'code1.ts'), 'test-code1.ts-reindexed');
     ...
     console.log(`[E2E Test] Query with no filter returned (excerpt): ${result.substring(0, 150)}...`);
  }, 10000);
