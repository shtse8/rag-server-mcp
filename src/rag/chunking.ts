...
function chunkGenericText(
  ...
  if (chunkSize <= overlap && chunkSize > 0) {
    console.warn(
      `[Chunking] Chunk size (${chunkSize.toString()}) should be greater than overlap (${overlap.toString()}). Adjusting overlap to ${String(Math.floor(chunkSize / 2))}`,
    );
    overlap = Math.floor(chunkSize / 2);
  } else if (chunkSize <= 0) {
    console.warn(
      `[Chunking] Chunk size (${chunkSize.toString()}) must be positive. Using default ${DEFAULT_CHUNK_SIZE.toString()}.`,
    );
  ...
  if (chunks.length > 0) {
    // const lastChunk = chunks[chunks.length - 1]; // Variable is unused
    ...
    let approxLastEnd = 0;
    if (i > 0) {
      // If the loop ran at least once
      const lastStep =
        chunkSize - overlap > 0 ? chunkSize - overlap : chunkSize;
...
function chunkMarkdown(content: string): Chunk[] {
  ...
  while ((match = codeBlockRegex.exec(content)) !== null) {
    ...
    if (codeBlockStartIndex > lastIndex) {
      ...
      if (textSegment.trim().length > 0) {
        console.log(
          `[Chunking] chunkMarkdown: Processing text segment [${lastIndex.toString()}-${codeBlockStartIndex.toString()}]`,
        );
    ...
    if (codeContent && codeContent.trim().length > 0) {
      console.log(
        `[Chunking] chunkMarkdown: Processing code block [${codeBlockStartIndex.toString()}-${codeBlockEndIndex.toString()}], lang: ${language ?? 'undefined'}`,
      );
  ...
  if (lastIndex < content.length) {
    ...
    if (remainingTextSegment.trim().length > 0) {
      console.log(
        `[Chunking] chunkMarkdown: Processing remaining text segment [${lastIndex.toString()}-END]`,
      );
  ...
  console.log(
    `[Chunking] chunkMarkdown finished, generated ${chunks.length.toString()} chunks.`,
  );
...
function chunkGenericCode(content: string, fileExtension: string): Chunk[] {
  ...
  const language = fileExtension.startsWith('.')
  ...
  console.log(
    `[Chunking] chunkGenericCode finished for ${fileExtension}, generated ${chunks.length.toString()} chunks.`,
  );
...
export function hierarchicalChunker(
  ...
  const extLower = fileExtension.startsWith('.')
    ? fileExtension.toLowerCase()
    : `.${fileExtension.toLowerCase()}`;
  const contentType = extensionToContentType[extLower] || 'unknown';

  console.log(
    `[Chunking] Processing file with extension ${extLower}, detected type: ${contentType}`,
  );
  ...
  try {
    if (contentType === 'text') {
      ...
      console.warn(
        `[Chunking] Unknown file type: ${extLower}. Using generic text chunking.`,
      );
      return chunkGenericText(content, 'unknown');
    }
  } catch (error) {
    console.error(
      `[Chunking] Error processing file with extension ${extLower}:`,
      error,
    );
...
export function chunkText(
  ...
  if (chunkSize <= overlap) {
    console.warn(
      `[Chunking] Chunk size (${chunkSize.toString()}) should be greater than overlap (${overlap.toString()}). Adjusting overlap to ${String(Math.floor(chunkSize / 2))}`, // Use Math.floor like in chunkGenericText
    );
    overlap = Math.floor(chunkSize / 2);
  }
  if (chunkSize <= 0) {
